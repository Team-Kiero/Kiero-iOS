...
struct MyPageView: View {
    ...
    init(
        ...
        onChildConnectionTap: @escaping () -> Void,
        onLogoutConfirm: @escaping () -> Void
    ) {
        _viewModel = StateObject(wrappedValue: viewModel)
        self.onChildConnectionTap = onChildConnectionTap
        self.onLogoutConfirm = onLogoutConfirm
    }
...
struct ConnectionChip: View {
    ...
    var body: some View {
        Text(count == 0 ? "연결 필요" : "\(count)명 연결됨")
            ...
            .overlay(
                Capsule()
                    .stroke(count == 0 ? .main : .gray500, lineWidth: 1)
            )
    }
}
