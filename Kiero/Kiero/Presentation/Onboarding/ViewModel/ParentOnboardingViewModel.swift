...
final class ParentOnboardingViewModel: BaseViewModel {
    ...
    private let parentInviteService: ParentInviteServiceType
    ...
    func generateInviteCode(childLastName: String, childFirstName: String) {
        ...
        Task { [weak self] in
            ...
            do {
                let data = try await parentInviteService.reissueInviteCode(
                    childLastName: last,
                    childFirstName: first
                )

                let sseToken = try await sseTokenRefresher.reissueSseAccessToken()
