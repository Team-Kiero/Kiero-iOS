...
final class TokenRefresher: TokenRefreshing {
    ...
    func refreshAccessToken() async throws {
        ...
        let task = Task { try await performRefreshAccessToken() }
        refreshTask = task
    ...
    private func performRefreshAccessToken() async throws {
        ...
        tokenStore.saveAccessToken(tokenData.accessToken)
    }

    func refreshAllTokens() async throws {
        if let task = refreshAllTask { return try await task.value }

        let task = Task { try await performRefreshAllTokens() }
        refreshAllTask = task
    ...
    func reissueSseAccessToken() async throws -> String {
        ...
        tokenStore.saveSseToken(tokenData.accessToken)
